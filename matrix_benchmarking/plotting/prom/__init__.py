...
class Plot():
    ...
    def do_lts_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        for entry in common.Matrix.all_lts_records(settings, setting_lists):
            ...
            entry_name = ", ".join([f"{key}={entry.metadata['settings'][key]}" for key in variables])

            sort_index = entry.metadata['settings'][ordered_vars[0]] if len(variables) == 1 \
                else entry_name
            ...
            for _metric in self.metrics:
                ...
                for metric in self.filter_metrics(entry, self.get_metrics(entry, metric_name)):
                    ...
                    if single_expe:
                        ...
                        entry_version = ", ".join([f"{key}={entry['metadata']['settings'][key]}" for key in variables])
