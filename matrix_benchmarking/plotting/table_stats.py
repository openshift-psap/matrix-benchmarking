...
class TableStats():
    ...
    def _register_stat(clazz, stat_obj):
        logging.info(f"Registering '{stat_obj.name}' ...")
    ...
    def process_mean_std(self, entry):
        ...
        if not values:
            logging.warning("{entry.location}.results.{self.field} is "
                  f"an empty list")
            return 0, 0
        ...
        for table_def, (table_name, table_rows) in entry.tables.items():
            ...
            try:
                ...
                try:
                    ...
                    logging.warning(f"props_to_hovergraph: Cannot find {table_simple_name}.msg_ts or time field ...")
                    return ""
    ...
    def do_plot(self, ordered_vars, params, param_lists, variables, cfg):
        ...
        if cfg_plot_mode:
            ...
            logging.info(f"Using var_length={var_length} instead of {len(variables)}")
        else:
